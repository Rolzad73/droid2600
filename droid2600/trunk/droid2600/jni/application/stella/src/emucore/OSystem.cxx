...
//
// $Id: OSystem.cxx,v 1.5 2010-12-30 08:33:46 cvs Exp $
//============================================================================
...
#ifdef ANDROID
// include Paddle.hxx so we can set the virtual screen width
#include "Paddles.hxx"
#endif

#ifdef ANDROID_PROFILE
#include "Profile.hxx"
#endif
...
#ifdef ANDROID
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void OSystem::setKeepAspectRatio(int val)
{
  keepAspectRatio = val;
}
#endif
...
void OSystem::mainLoop()
{
  ...
  {
    uInt64 fpsCheckTime = getTicks();
    int longFrameCount = 0;
    bool skipMode = false;
    // Sleep-based wait: good for CPU, bad for graphical sync
    for(;;)
    {
#ifdef ANDROID_PROFILE
tslib::Profile::getInstance()->start();
#endif
...
#ifdef ANDROID_PROFILE
    tslib::Profile::getInstance()->startObservation("OSystem::mainLoop (wait)");
#endif
...
#ifdef ANDROID_PROFILE
    tslib::Profile::getInstance()->stopObservation("OSystem::mainLoop (wait)");
#endif

      myTimingInfo.totalTime += (getTicks() - myTimingInfo.start);
      myTimingInfo.totalFrames++;
#ifdef ANDROID_PROFILE
tslib::Profile::getInstance()->stop();
#endif
    }
  }
  else
...
bool OSystem::queryVideoHardware()
{
  ...
  if(SDL_WasInit(SDL_INIT_VIDEO) == 0)
...
#ifdef ANDROID
    // ask SDL about the current resolution.
    ...
    // Trent - Through trial and error I have found games (miner2049er v2) that 
    // required up to 250 vertical resolution.
    assert(myDesktopWidth >= 320 && myDesktopHeight >= 250);

    // android desktops have big resolutions but are small in size. We set 
    // the desktop width to the minimum possible while keeping the aspect 
    // ratio if desired.
    if (myDesktopWidth > myDesktopHeight) {
        // landscape mode
        if (keepAspectRatio == 1) {
            myDesktopWidth = (((info->current_w*250)/info->current_h)/2)*2;
            myDesktopHeight = 250;
        }
        else {
            myDesktopWidth  = 320;
        ...
        if (keepAspectRatio == 1) {
            myDesktopWidth = 320;
            myDesktopHeight = (((320*info->current_h)/info->current_w)/2)*2;
        }
        else {
            myDesktopWidth  = 320;
            myDesktopHeight = 250;
    ...
    __android_log_print(ANDROID_LOG_INFO, "OSystem::queryVideoHardware",
        "reset resolution w=%i h=%i", myDesktopWidth, myDesktopHeight);

   Paddles::setScreenWidth(myDesktopWidth);
