...
//
// $Id: LauncherDialog.cxx,v 1.1.1.1 2010-10-26 03:44:58 cvs Exp $
//
...
{
#ifdef ANDROID
    // look for a stored romdir in the following order of preference:
    //     1) from "romdir" configuration
    //     1) parent directory of the lastrom rom file
    //     2) /sdcard
    //     3) /
    string romdir("/"); // hope it doesn't come to this :-(
    const string& loadedRomDir = instance().settings().getString("romdir");
//__android_log_print(ANDROID_LOG_INFO, "Stella", "loadedRomDir=%s", loadedRomDir.c_str());
    if (loadedRomDir == "") {
        // no romdir set. check to see if lastrom is set
        const string& lastRom = instance().settings().getString("lastrom_dir");
//__android_log_print(ANDROID_LOG_INFO, "Stella", "lastrom_dir=%s", lastRom.c_str());
        if (lastRom == "") {
            // no romdir or lastrom, finally try /sdcard/Droid2600/roms if available
            FilesystemNode n("/sdcard/Droid2600/roms");
            if (n.exists() && n.isDirectory()) {
                romdir = string("/sdcard/Droid2600/roms");
            }
            else {
                FilesystemNode n2("/sdcard");
                if (n2.exists() && n2.isDirectory()) {
                    romdir = string("/sdcard");
                }
            }
        }
        else {
            FilesystemNode fsNode(lastRom);
            if (fsNode.exists() && fsNode.isDirectory()) {
                romdir = string(fsNode.getPath());
            }
        }
    }
    else {
        romdir = string(loadedRomDir);
    }
#else
    const string& romdir = instance().settings().getString("romdir");
#endif
 
// __android_log_print(ANDROID_LOG_INFO, "Stella", "romdir=%s", romdir.c_str());
      ...
      if(item >= 0)
      {
        ...
        else
        {
          if(LauncherFilterDialog::isValidRomName(rom, extension))
          {
            ...
            #endif
#ifdef ANDROID // Make sure the settings get saved to file
              instance().settings().setString("lastrom_dir", myCurrentNode.getPath());
