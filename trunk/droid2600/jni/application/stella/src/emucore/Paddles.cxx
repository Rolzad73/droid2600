...
//
// $Id: Paddles.cxx,v 1.2 2010-12-30 08:33:46 cvs Exp $
//============================================================================
...
void Paddles::update()
{
  ...
  // Mouse events
#ifdef ANDROID
  int newx = myEvent.get(Event::MouseAxisXValue);
#endif
  if(myJack == Left && (_MOUSEX_PADDLE == 0 || _MOUSEX_PADDLE == 1) && newx > 0)
  {
    // TODO - add infrastructure to map mouse direction to increase or decrease charge
#ifdef ANDROID
    myCharge[_MOUSEX_PADDLE] = (TRIGMAX-myEvent.get(Event::MouseAxisXValue));
#else
  ...
  else if(myJack == Right && (_MOUSEX_PADDLE == 2 || _MOUSEX_PADDLE == 3) && newx > 0)
  {
    // TODO - add infrastructure to map mouse direction to increase or decrease charge
#ifdef ANDROID
    myCharge[_MOUSEX_PADDLE-2] = (TRIGMAX-myEvent.get(Event::MouseAxisXValue));
#else
...
#ifdef ANDROID
//if (newx > 0) {
//__android_log_print(ANDROID_LOG_INFO, "Paddles", "%i, %i, %i", newx,
//(Int32) (675294*myCharge[1])/320,
//(675294*myCharge[0])/320);
//}
  myAnalogPinValue[Five] = 411507 + ((675294*(myCharge[1] - TRIGMIDDLE))/320);
  myAnalogPinValue[Nine] = 411507 + ((675294*(myCharge[0] - TRIGMIDDLE))/320);
#else
...
#ifdef ANDROID
void Paddles::setScreenWidth(int width) {
    TRIGMAX = width;
    TRIGMIDDLE = TRIGMAX/2;
}    
#endif
...
#ifdef ANDROID
int Paddles::TRIGMAX = 320;
int Paddles::TRIGMIDDLE = 160;
#endif
