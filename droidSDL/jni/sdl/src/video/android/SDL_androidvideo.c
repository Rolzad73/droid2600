...
JAVA_EXPORT_NAME(DemoRenderer_nativeResize) ( JNIEnv*  env, jobject  thiz, jint w, jint h )
{
    ...
    __android_log_print(ANDROID_LOG_INFO, "libSDL", "Physical screen resolution is %dx%d", SDL_ANDROID_sWindowWidth, SDL_ANDROID_sWindowHeight);
}
...
JNIEXPORT void JNICALL 
JAVA_EXPORT_NAME(SDLInterface_nativeQuit) ( JNIEnv*  env, jobject  thiz )
{
//	__android_log_print(ANDROID_LOG_INFO, "DroidSDL.native", "before SDL_PrivateQuit");
	int result = SDL_PrivateQuit();
//	__android_log_print(ANDROID_LOG_INFO, "DroidSDL.native", "after SDL_PrivateQuit %i", result);
}
...
{
	JavaEnv = env;
    JavaRenderer = (*JavaEnv)->NewGlobalRef(env, thiz);
