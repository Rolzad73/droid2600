...
#ifdef ANDROID
    // we are using a soft surface in android so we do not want to have to 
    // check a static list of supported modes. This bypasses that and 
    flags |= SDL_ANYFORMAT;
    flags &= ~SDL_HWPALETTE;
    video_w = width;
    video_h = height;
    video_bpp = SDL_VideoSurface->format->BitsPerPixel;
    is_opengl = 0;
