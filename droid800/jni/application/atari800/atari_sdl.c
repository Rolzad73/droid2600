...
#ifdef ANDROID
static int android_show_border = 0;
static int keep_aspect_ratio = 0;
static int android_virtual_screen_width = 0;
static int android_virtual_screen_width_pos_delta = 0;
static int android_virtual_screen_height = 0;
static int android_virtual_screen_height_pos_delta = 0;
#endif
...
static int KBD_STICK_0_LEFT = SDLK_LEFT;
static int KBD_STICK_0_RIGHT = SDLK_RIGHT;
static int KBD_STICK_0_DOWN = SDLK_DOWN;
static int KBD_STICK_0_UP = SDLK_UP;
#else
	...
	for (i = 0; i < num_modes; i++) {
        ...
        if (keep_aspect_ratio == 1) {
            ...
            if (modes[i]->w > modes[i]->h) {
                ...
                android_virtual_screen_width_pos_delta = (resolutions[i].width - width)/2;
            }
            else {
                // portrait
                resolutions[i].width = width;
                resolutions[i].height = (((width*modes[i]->h)/modes[i]->w)/2)*2;

                android_virtual_screen_height_pos_delta = (resolutions[i].height * .02);
            }
        }
        else {
            resolutions[i].width = width;
...
#ifdef ANDROID
    /* we want to select menu items with the trigger, so if the ui is and we
     * have a trigger event - convert it to a space.
     */
    if (UI_is_active && lastkey == KBD_TRIG_0) {
        lastkey = SDLK_SPACE;
    }
#endif
	...
	for (i = j = 1; i < *argc; i++) {
		...
		if (strcmp(argv[i], "-scanlines") == 0) {
			...
			keep_aspect_ratio = 1;
                ...
                else if (strcmp(argv[i], "-showborder") == 0) {
                    android_show_border = 1;
                }
#endif
		else {
			if (strcmp(argv[i], "-help") == 0) {
