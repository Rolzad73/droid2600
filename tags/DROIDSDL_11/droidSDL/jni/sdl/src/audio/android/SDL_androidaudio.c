...
//		__android_log_print(ANDROID_LOG_ERROR, "libSDL", "Application requested unsupported audio format - only S8 and S16 are supported");
	...
	__android_log_print(ANDROID_LOG_INFO, "SDL_androidaudio.c", 
        "ANDROIDAUD_OpenAudio DEBUG AUDIO: channels %d samples=%d bytespersample=%d freq=%d format=%d bufferSize=%d", 
        (jint)audioFormat->channels, audioFormat->samples, bytesPerSample, audioFormat->freq,  
        audioFormat->format, audioBufferSize);
